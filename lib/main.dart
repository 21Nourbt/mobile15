...
// import 'package:flutter_application_1/login_screen.dart';
// import 'package:flutter_application_1/main_layout.dart';
// import 'package:flutter_application_1/notification_dropdown.dart';
// import 'package:flutter_application_1/officeRoomList.dart';
...
//         '/booking-history': (context) => const ReservationTableScreen(),
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      routes: {
        ...
        // Main Application Screens with MainLayout
        '/home': (_) => MainLayout(child: const CoworkingApp(), currentIndex: 0),
        '/reserveMeetingRoom': (_) => MainLayout(child: const CategoryRoomList(), currentIndex: 1),
        '/assistant': (_) => MainLayout(child: const ChatScreen(), currentIndex: 1),
        '/openspace': (_) => MainLayout(child: const FloorPlanBookingPage(), currentIndex: 1),
          ...
          if (args != null) {
            ...
            return MaterialPageRoute(
              builder: (_) => MainLayout(
                child: ReservationScreen(room: room, reservations: reservations),
                currentIndex: 1,
              ),
            );
          }
        } else if (settings.name == '/course-detail1') {
          final args = settings.arguments as Map<String, dynamic>?;
          if (args != null) {
            final room = args['room'] as Map<String, dynamic>;
            final reservations = (args['reservations'] as List?)?.cast<Map<String, dynamic>>() ?? [];
            return MaterialPageRoute(
              builder: (_) => MainLayout(
                child: OfficeRoomReservationScreen(room: room, reservations: reservations),
                currentIndex: 1,
              ),
            );
          }
